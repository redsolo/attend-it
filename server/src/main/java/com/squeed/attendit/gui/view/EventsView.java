package com.squeed.attendit.gui.view;
...
public class EventsView extends VerticalLayout {
	...
	private String[] fields = new String[]{"title", "date", "currentRegistrations", "maxRegistration"};

	private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");

    public EventsView(RegistrationDAO dao) {
        ...
        // set column headers
        table.setColumnHeaders(new String[] { "Event Name", "Date", "Current registrations", "Max registrations" });
...
//        table.setColumnCollapsed(ExampleUtil.iso3166_PROPERTY_FLAG, true);
	...
	private void loadEvents() {
		...
		List<EventInstance> events = dao.getEvents();
		for(EventInstance event : events) {
			Object item = ic.addItem();
			ic.getContainerProperty(item, fields[0]).setValue(event.getTitle());
            ic.getContainerProperty(item, fields[1]).setValue(sdf.format(event.getEventDateTime().getTime()));
            ic.getContainerProperty(item, fields[2]).setValue(event.getRegistrations().size());
            ic.getContainerProperty(item, fields[3]).setValue(event.getMaxAttendants());
		}
	}
